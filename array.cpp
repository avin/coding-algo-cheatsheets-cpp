...
void example_vector() {
    ...
    // 2. Инициализация списком (initializer_list)
    std::vector<int> v2 = {1, 2, 3, 4, 5};

    // 3. Вектор заданного размера, все элементы инициализируются значением по умолчанию (0)
    std::vector<int> v3(5); // {0, 0, 0, 0, 0}

    // 4. Вектор заданного размера с заданным значением для всех элементов
    std::vector<int> v4(5, 42); // {42, 42, 42, 42, 42}

    // 5. Копирование из другого вектора
    std::vector<int> v5(v2); // копия v2

    // 6. Перемещение (move) из другого вектора
    std::vector<int> temp = {7, 8, 9};
    std::vector<int> v6(std::move(temp)); // temp теперь пустой

    // 7. Конструктор из диапазона (из массива C)
    int arr[] = {10, 11, 12, 13};
    std::vector<int> v7(std::begin(arr), std::end(arr)); // {10,11,12,13}

    // 8. Конструктор из диапазона (из другого контейнера: например, из списка std::list)
    std::list<int> lst = {20, 21, 22};
    std::vector<int> v8(std::begin(lst), std::end(lst)); // {20,21,22}

    // 9. Использование assign для перезаписи значений вектора
    std::vector<int> v9;
    v9.assign(3, 99);       // {99, 99, 99}
    v9.assign(v2.begin(), v2.begin() + 3); // копирует первые 3 элемента из v2: {1,2,3}

    // 10. Резервирование памяти без изменения размера
    std::vector<int> v10;
    v10.reserve(100);      // зарезервировать память под 100 элементов
    // после reserve размер остаётся 0, но ёмкость >= 100

    // 11. Изменение размера с инициализацией новых элементов
    std::vector<int> v11 = {1, 2, 3};
    v11.resize(5);         // теперь {1,2,3,0,0}
    v11.resize(2);         // теперь {1,2}

    // 12. Инициализация с помощью итераторов алгоритма std::iota
